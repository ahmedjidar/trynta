...
function main(): void {
   ...
   */
  const written = new Map<string, { variants: Map<string, string>; chosen: Candidate }>();
  ...
  for (const [key, list] of byKey) {
    ...
    if (chosen !== null) written.set(key, { variants, chosen });
  }
  ...
  if (!reportOnly) {
    const lines: string[] = [
      ...
      '# kind\tmatch\tkey\tsource\tvariant\tlicence\tbrand_hex',
    ];
    const emit = (kind: string, map: Map<string, MapRow>) => {
      for (const key of [...map.keys()].sort()) {
        ...
        lines.push(
          `${kind}\t${key}\t${r.key}\t${r.source}\t${r.variant}\t${r.licence}\t${r.brandHex}`,
        );
      }
    };
    emit('host', hostRows);
